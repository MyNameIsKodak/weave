...
void CVisuals::DrawWatermark() {
	std::string output = str("HVHOOK | ") + csgo->username;
	output += str(" | beta");
...
public:
	...
	void render(const Vector2D& pos) {
		...
		for (auto& el : elements) {
			...
			g_Render->FilledRectGradient(pos.x - 5.f + text_size.y, pos.y + 28.f + cursor, 25 ,15,color_t(0, 0, 0, 0), color_t(0, 0, 0, 100), color_t(0, 0, 0, 100), color_t(0, 0, 0, 0));
			g_Render->FilledRectGradient(pos.x + 19.f + text_size.y, pos.y + 28.f + cursor, 16,15, color_t(0, 0, 0, 100), color_t(0, 0, 0, 0), color_t(0, 0, 0, 0), color_t(0, 0, 0, 100));

			g_Render->DrawString(pos.x + 3.f + text_size.y, pos.y + 36.f + cursor, color_t(0, 0, 0).manage_alpha(255.f * easeOutQuad(current_animation)), render::centered_y, fonts::menu_desc, str("%s"), el.second.name.c_str());

			g_Render->DrawString(pos.x + 4.f + text_size.y, pos.y + 35.f + cursor, color_t(123, 194, 21).manage_alpha(255.f * easeOutQuad(current_animation)),render::centered_y , fonts::menu_desc,str("%s"), el.second.name.c_str());
...
std::string GetBindName(int id) {
	...
	case bind_double_tap:      return str("DT");
	case bind_hide_shots:      return str("HS");
	case bind_aa_inverter:     return str("INVERTER");
	case bind_manual_left:     return str("Left");
	case bind_manual_right:    return str("Right");
	case bind_manual_back:     return str("Back");
	case bind_manual_forward:  return str("Forward");
	case bind_fake_duck:       return str("DUCK");
	case bind_slow_walk:       return str("SLOW-WALK");
	case bind_peek_assist:     return str("QUICK-PEEK");
	default: return str("?");
	}
}
...
void DrawIndicators() {
	...
	Vector2D size = Vector2D(50.f, 25.f);
	...
	if (bind_list.last_size > 0) {
		...
		//g_Render->FilledRect(position.x, position.y, size.x, size.y, color_t(46,46,46));
		...
		// Text shadow
		g_Render->DrawString(position.x-1, position.y-1, color_t(0, 0, 0), render::centered_y, fonts::menu_desc, str("KEYBINDS"));

		// Text
		g_Render->DrawString(position.x, position.y, color_t(123, 194, 21), render::centered_y, fonts::menu_desc, str("KEYBINDS"));
