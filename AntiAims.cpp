...
void CAntiAim::Fakelag()
{
	...
	if (!CanTriggerFakeLag()) {
		...
		csgo->max_fakelag_choke = clamp(vars.antiaim.fakelagfactor, 1,
			((vars.misc.restrict_type == 0 || csgo->game_rules->IsValveDS()) ? 6 : 62));
...
void CAntiAim::Yaw(bool legit_aa)
{
	...
	const float desync_amount =vars.antiaim.desync_amount;
	const float yaw_amount = vars.antiaim.yaw_amount;
	...
	if (vars.antiaim.desync)
	{
		...
		if (!csgo->send_packet)
		{
			...
			csgo->cmd->viewangles.y += csgo->SwitchAA ? desync_amount : -desync_amount;
		}
		else 
			csgo->cmd->viewangles.y += csgo->SwitchAA ? yaw_amount : -yaw_amount;
	}
