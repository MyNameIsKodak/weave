...
void c_combo::render() {
	...
	if (label.size() > 0) {
		...
		g_Render->DrawString(pos.x, pos.y, color_t(200 + 55.f * animation, 200 + 55.f * animation, 200 + 55.f * animation, alpha),
			render::none | render::outline, fonts::menu_main, label.c_str());
		pos.y += after_text_offset;
		if (this == wnd->g_active_element)
			c->unlock_bounds();
	}
	if (open) {
		...
		g_Render->DrawString(pos.x + 10, pos.y + size.y / 2, color_t(255, 255, 255, alpha), render::centered_y | render::outline,
			fonts::menu_main, elements[*(int*)value].c_str());
		...
		for (size_t i = 0; i < elements.size(); i++) {
			...
			g_Render->DrawString(pos.x + 10, pos.y + size.y / 2, *(int*)value == i ? color_t(110, 184, 13, new_alpha) : clr2, render::centered_y| render::outline, fonts::menu_main, elements[i].c_str());
		}
	}
	else {
		...
		g_Render->FilledRectGradient(pos.x, pos.y, size.x, size.y, clr2, clr2, clr, clr);
		...
		g_Render->DrawString(pos.x + 10, pos.y + size.y / 2, color_t(200 + 55.f * animation, 200 + 55.f * animation, 200 + 55.f * animation, alpha),
			render::centered_y | render::outline, fonts::menu_main, elements[*(int*)value].c_str());
