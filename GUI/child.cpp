...
void c_child::render() {
	...
	auto clr = color_t(30, 30, 30, alpha); 
		g_Render->FilledRect(pos.x-1, pos.y+1, size.x+2, size.y-2, color_t(20, 20, 20, (int)(wnd->get_transparency() * 2.55f)));
	g_Render->FilledRect(pos.x, pos.y, size.x, size.y, color_t(23, 23, 23, (int)(wnd->get_transparency() * 2.55f)));
	...
	if (should_draw_scroll) {
		...
		if (scroll.value > 0) {
			g_Render->FilledRectGradient(pos.x, pos.y, size.x, 20,
		...
		if (scroll.value < max_scroll_value) {
			g_Render->FilledRectGradient(pos.x, pos.y + size.y - 20, size.x, 20,
				color_t(25, 25, 25, 0), color_t(25, 25, 25, 0),
				color_t(25, 25, 25, alpha), color_t(25, 25, 25, alpha));
	...
	g_Render->DrawString(pos.x - 50 + size.x / 2, pos.y, color_t(255, 255, 255, alpha),
		render::centered_y | render::centered_x | render::outline, fonts::menu_main, get_title().c_str(), total_elements_size, scroll);
}
