...
void CRagebot::Run()
{
	...
	dt_ready = g_Binds[bind_double_tap].active && g_Misc->dt_bullets <= 1 ;
		...
		if (vars.ragebot.autoshoot) {
			...
			if (hitchance && can_shoot_if_fakeduck && is_able_to_shoot)
			{
				if (!csgo->fake_duck) {
					...
					csgo->max_fakelag_choke = CanExploit() ? 0.1 : 0.1;
				}
				if (force_accuracy)
					csgo->cmd->buttons |= IN_ATTACK;
			}
		...
		if (hitchance && can_shoot_if_fakeduck && is_able_to_shoot)
		{
			if (csgo->cmd->buttons & IN_ATTACK) {
				...
				if (vars.visuals.chams[enemy_ragebot_shot].enable) {
					...
					// âîò ýòó õóéíþ ðàññêîìåí÷èâàòü êîãäà íàäî òåñò àíèìôèêñà + ñåéôïîèíòîâ, ÷òîá âèäåòü êóäà ðåéäæ ñòðåëÿåò
