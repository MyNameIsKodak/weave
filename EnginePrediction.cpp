...
void CNetData::Apply() {
    ...
    // set data.
    if (std::abs(punch_delta.x) < 0.03120f &&
        std::abs(punch_delta.y) < 0.03120f &&
        std::abs(punch_delta.z) < 0.03120f)
        csgo->local->GetPunchAngle() = data->m_punch;

    if (std::abs(punch_vel_delta.x) < 0.03120f &&
        std::abs(punch_vel_delta.y) < 0.03120f &&
        std::abs(punch_vel_delta.z) < 0.03120f)
        csgo->local->GetPunchAngleVel() = data->m_punch_vel;

    if (std::abs(view_delta.x) < 0.03120f &&
        std::abs(view_delta.y) < 0.03120f &&
        std::abs(view_delta.z) < 0.03120f)
        csgo->local->GetVecViewOffset() = data->m_view_offset;

    if (std::abs(modifier_delta) < 0.03120f)
